    ...
    public function up(): void
    {
        ...
        if (isset($detailIdsByName['Locally Stranded Individuals (LSI)'])) {
            ...
            $upsert(
                ...
                'Upload a valid ID of the client or person to be interviewed. If there is no valid ID, upload either a medical certificate or a travel authority issued by the Philippine National Police (PNP) as proof of identity.',
                true,
                10
            );
        }

        if (isset($detailIdsByName['Food Assistance'])) {
            $detailId = $detailIdsByName['Food Assistance'];
            $addCommonIdRequirement($detailId);
            $upsert(
                $detailId,
            ...
            $upsert(
                $detailId,
                'Medical Document as Proof that the Beneficiary is Admitted',
                'Upload a medical document only if it applies to the request and is needed to prove that the beneficiary is admitted.',
                false,
                40
            );
        }

        if (isset($detailIdsByName['Educational Assistance'])) {
            $detailId = $detailIdsByName['Educational Assistance'];
            $addCommonIdRequirement($detailId);
            $upsert(
                $detailId,
                'Validated School ID',
                'Upload the validated school ID of the beneficiary or student.',
                true,
                30
            );
            $upsert(
                $detailId,
                'Enrollment Assessment Form / Certificate of Enrollment or Registration / Statement of Account',
                'Upload any one of the following: enrollment assessment form, certificate of enrollment or registration, or statement of account.',
                true,
                40
            );
        }
