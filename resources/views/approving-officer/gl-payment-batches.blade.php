...
<main class="p-8 max-w-7xl mx-auto space-y-6">
    <section class="rounded-[28px] bg-[radial-gradient(circle_at_top_left,_rgba(255,255,255,0.16),_transparent_35%),linear-gradient(135deg,_#234E70_0%,_#18384f_46%,_#27597c_100%)] px-8 py-9 text-white shadow-[0_24px_60px_rgba(24,56,79,0.18)]">
        <div class="grid gap-6 lg:grid-cols-[1.4fr_.9fr] lg:items-end">
            <div>
                ...
                <h1 class="mt-5 text-3xl font-bold leading-tight sm:text-4xl">GL Program Approval Batches</h1>
                <p class="mt-3 max-w-2xl text-sm leading-6 text-white/80 sm:text-base">
                    Approve grouped GL finance packets while still opening and inspecting each included record one by one.
                </p>
            </div>
    ...
    <section class="rounded-3xl border border-slate-200 bg-white p-3 shadow-sm">
        <div class="flex flex-wrap gap-3">
            <a href="{{ route('approving.gl-payment-approvals', ['scope' => 'active']) }}"
            ...
            <a href="{{ route('approving.gl-payment-approvals', ['scope' => 'finished']) }}"
    ...
    <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <form method="GET" action="{{ route('approving.gl-payment-approvals') }}" class="grid gap-4 md:grid-cols-4">
                ...
                <select name="payment_status" class="input">
                    ...
                    @foreach($paymentStatusOptions as $paymentStatus)
                        <option value="{{ $paymentStatus }}" @selected($filters['payment_status'] === $paymentStatus)>{{ match ($paymentStatus) {
                            'draft' => 'For Processing (Program Approval)',
            ...
            <div class="flex items-end gap-3">
                ...
                <a href="{{ route('approving.gl-payment-approvals') }}" class="btn-secondary">Reset</a>
            </div>
        </form>
    </section>
            ...
            <table class="min-w-full text-left">
                ...
                <tbody class="divide-y divide-slate-200">
                    @forelse($batches as $batch)
                        @php
                            $isHistoricalRow = (int) ($batch->program_approved_by ?? 0) === (int) auth()->id()
                                && ! in_array($batch->program_approval_status, ['pending_approval', null], true);
                        ...
                        <tr class="hover:bg-slate-50/70">
                            <td class="px-5 py-4 align-middle">
                                ...
                                <p class="mt-1 text-xs text-slate-500">
                                    {{ $isHistoricalRow ? 'Handled' : 'Created' }}
                            ...
                            <td class="px-5 py-4 text-right">
                                <a href="{{ route('approving.gl-payment-approvals.show', $batch->id) }}"
                                   class="inline-flex items-center rounded-xl bg-[#234E70] px-4 py-2 text-sm font-semibold text-white hover:bg-[#18384f]">
                                    {{ $isFinishedScope || $isHistoricalRow ? 'View Batch' : 'Review Batch' }}
                                </a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="8" class="px-5 py-12 text-center text-sm text-slate-500">
                                No GL finance batches are waiting right now.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
