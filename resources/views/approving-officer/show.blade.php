...
@php
    ...
    $incomeSources = collect($application->income_sources ?? [])->filter(fn ($row) => filled($row['source'] ?? null) || filled($row['amount'] ?? null));
    $incomeSourceLabels = [
    ...
    $pageTitle = ($readOnly ?? false) ? 'Application Details' : 'Final Review';
    ...
    <div class="card">
        ...
        <div id="tab-intake" class="tab-panel" role="tabpanel" hidden>
            ...
            <div class="section-card">
                ...
                <div class="space-y-2">
                    @foreach($incomeSourceLabels as $label)
                        @php
                            $incomeRow = collect($application->income_sources ?? [])->firstWhere('source', $label) ?? [];
                        @endphp
            ...
            <div class="section-card">
                ...
                @php($externalResourcesItems = $multiValueSections['External Resources'] ?? [])
            ...
            <div class="section-card">
                ...
                @php($selfHelpItems = $multiValueSections['Self-help Efforts'] ?? [])
            ...
            <div class="section-card">
                ...
                <div class="grid gap-3 text-sm md:grid-cols-2 xl:grid-cols-3">
                    ...
                    <div class="flag-item"><span class="muted">Has Insurance Coverage</span><br>{{ $application->has_insurance_coverage ? 'Yes' : 'No' }}</div>
                    <div class="flag-item"><span class="muted">Has Savings</span><br>{{ $application->has_savings ? 'Yes' : 'No' }}</div>
                </div>
            </div>
            ...
            <div class="section-card">
                ...
                @php($clientSectorItems = $multiValueSections['Client Sectors'] ?? [])
                @php($clientSubCategoryItems = $multiValueSections['Client Sub-categories'] ?? [])
                @php($disabilityTypeItems = $multiValueSections['Disability Types'] ?? [])
                @php($recentCrisisItems = $multiValueSections['Recent Crisis Types'] ?? [])
                @php($supportSystemItems = $multiValueSections['Support Systems'] ?? [])
                ...
                <div class="section-card">
                    ...
                    <div class="grid gap-6 lg:grid-cols-2">
                        <form method="POST"
                              ...
                              class="rounded-2xl border border-emerald-100 bg-emerald-50/60 p-5 space-y-4">
                            @csrf

                            <div>
                                <p class="text-sm font-semibold text-emerald-900">Approve Application</p>
                        ...
                        <form method="POST"
                              ...
                              class="rounded-2xl border border-rose-100 bg-rose-50/60 p-5 space-y-4">
                            @csrf

                            <div>
                                <p class="text-sm font-semibold text-rose-900">Deny Application</p>
                                <p class="mt-1 text-sm text-rose-800">Provide a clear reason so the case history stays complete.</p>
                            </div>
