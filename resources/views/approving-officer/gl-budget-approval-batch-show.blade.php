...
<main class="space-y-6">
    <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <a href="{{ route('budget-approver.gl-payment-approvals') }}" class="text-sm text-slate-500 hover:text-[#234E70]">
                    &larr; Back to Budget Approval Batches
                </a>
                <p class="mt-4 text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Budget Approver Batch Workspace</p>
                <h1 class="mt-2 text-3xl font-black text-sky-950">{{ $batch->batch_no }}</h1>
                <p class="mt-2 text-sm text-slate-500">
                    Inspect the included GL records one by one, then approve or return the whole batch.
                </p>
            </div>
    ...
    <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div class="mt-3">
                <span class="inline-flex rounded-full border border-violet-200 bg-violet-50 px-3 py-1 text-[11px] font-black uppercase tracking-[0.16em] text-violet-700">
                    {{ $batch->status === 'for_compliance_budget_officer' ? 'For Compliance (Budget Officer)' : ($batch->status === 'for_processing_accounting' ? 'For Processing (Accounting)' : ($batch->status === 'disapproved' ? 'Disapproved' : 'For Processing (Budget)')) }}
                </span>
            </div>
        </article>
        <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
    ...
    <section class="grid gap-6 xl:grid-cols-[minmax(0,2fr)_380px]">
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white">
                        @foreach($batch->applications as $application)
                            <tr>
                                ...
                                <td class="px-4 py-4 align-top">
                                    <a href="{{ route('budget-approver.gl-payment-approvals.records.show', [$batch->id, $application->id]) }}"
        ...
        <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <form method="POST" action="{{ route('budget-approver.gl-payment-approvals.update', $batch->id) }}" class="mt-6 space-y-4">
                @csrf
                @method('PATCH')
                ...
                <div>
                    ...
                    <textarea name="remarks" class="input h-32" placeholder="Add the reason for compliance return or disapproval.">{{ old('remarks', $batch->budget_approval_remarks) }}</textarea>
