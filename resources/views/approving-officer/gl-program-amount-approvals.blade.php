    ...
    <section class="rounded-[28px] bg-[radial-gradient(circle_at_top_left,_rgba(255,255,255,0.16),_transparent_35%),linear-gradient(135deg,_#234E70_0%,_#18384f_46%,_#27597c_100%)] px-8 py-9 text-white shadow-[0_24px_60px_rgba(24,56,79,0.18)]">
        <div class="grid gap-6 lg:grid-cols-[1.4fr_.9fr] lg:items-end">
            <div>
                ...
                <h1 class="mt-5 text-3xl font-bold leading-tight sm:text-4xl">Program Amount Approval Queue</h1>
                <p class="mt-3 max-w-2xl text-sm leading-6 text-white/80 sm:text-base">
                    Review guarantee letter cases that already passed accounting and are now waiting for final program amount approval.
                </p>
            </div>
            ...
            <table class="min-w-full text-left">
                ...
                <tbody class="divide-y divide-slate-200">
                    @forelse($applications as $application)
                        @php
                            $isHistoricalRow = (int) ($application->gl_program_amount_approved_by ?? 0) === (int) auth()->id()
                                && ! in_array($application->gl_program_amount_approval_status, ['pending_approval', null], true);

                            $rowStatusLabel = match (true) {
                                $application->gl_program_amount_approval_status === 'disapproved' => 'Disapproved',
                        ...
                        <tr class="hover:bg-slate-50/70">
                            <td class="px-5 py-4">
                                ...
                                <p class="mt-1 text-xs text-slate-500">
                                    {{ $isHistoricalRow ? 'Handled' : 'Accounting approved' }} {{ $application->gl_accounting_approved_at?->format('M d, Y h:i A') ?? $application->updated_at?->format('M d, Y h:i A') }}
                                </p>
                            </td>
                            <td class="px-5 py-4">
                                <p class="font-semibold text-slate-900">{{ trim(($application->client?->first_name ?? '').' '.($application->client?->last_name ?? '')) ?: '-' }}</p>
                                @if($application->gl_accounting_remarks)
                            ...
                            <td class="px-5 py-4 text-right">
                                <a href="{{ route('approving.gl-program-amount-approvals.show', $application->id) }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-5 py-12 text-center text-sm text-slate-500">
                                No program amount approvals are waiting right now.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
