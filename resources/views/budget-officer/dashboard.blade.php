...
<main class="space-y-6">
    <section class="overflow-hidden rounded-[32px] border border-slate-200 bg-[radial-gradient(circle_at_top_left,_rgba(221,214,254,.55),_transparent_28%),linear-gradient(135deg,_#ffffff_0%,_#f5f3ff_48%,_#f8fafc_100%)] p-6 shadow-sm lg:p-8">
        <div class="grid gap-6 xl:grid-cols-[minmax(0,1.25fr)_340px] xl:items-end">
            <div>
                <p class="text-xs font-black uppercase tracking-[0.22em] text-violet-700">Budget Officer</p>
                ...
                <div class="mt-6 flex flex-wrap gap-3">
                    <a href="{{ route('budget-officer.gl-payment-approvals') }}"
    ...
    <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <article class="rounded-3xl border border-violet-200 bg-violet-50 p-5 shadow-sm">
            <span class="inline-flex rounded-full border border-violet-200 bg-white px-3 py-1 text-[11px] font-black uppercase tracking-[0.16em] text-violet-700">For Review</span>
            <p class="mt-3 text-3xl font-black text-violet-950">{{ $stats['for_review'] }}</p>
            <p class="mt-2 text-sm text-violet-800">Approved GL cases now routed to budget processing.</p>
        </article>
        ...
        <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div class="flex items-center justify-between gap-3">
                <div>
                    ...
                    <h2 class="mt-2 text-2xl font-black text-slate-950">Recently Approved for Budget</h2>
                </div>
                <a href="{{ route('budget-officer.gl-payment-approvals') }}"
                        ...
                        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                            <div>
                                ...
                                <div class="mt-3 flex flex-wrap gap-3 text-xs font-semibold text-slate-500">
                                    ...
                                    <span>Approved {{ optional($application->gl_program_approved_at)->format('M d, Y h:i A') ?? '-' }}</span>
                                </div>
                            </div>
                            ...
                            <div class="text-left lg:text-right">
                                ...
                                <a href="{{ route('budget-officer.gl-payment-approvals.show', $application->id) }}"
