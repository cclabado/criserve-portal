...
@php
    ...
    $listRoute = $isOfficer ? 'accounting-officer.gl-payment-reviews' : 'accounting-approver.gl-payment-approvals';
    $listLabel = $isOfficer ? 'Accounting Review Batches' : 'Accounting Approval Batches';
    $cashCertificationRoute = 'accounting-approver.cash-certifications';
@endphp
...
<main class="space-y-6">
    <section class="overflow-hidden rounded-[32px] border border-slate-200 bg-[radial-gradient(circle_at_top_left,_rgba(191,219,254,.55),_transparent_28%),linear-gradient(135deg,_#ffffff_0%,_#eef6ff_48%,_#f8fafc_100%)] p-6 shadow-sm lg:p-8">
        <div class="grid gap-6 xl:grid-cols-[minmax(0,1.25fr)_340px] xl:items-end">
            <div>
                <p class="text-xs font-black uppercase tracking-[0.22em] text-sky-700">{{ $workspaceLabel }}</p>
                <h1 class="mt-3 text-4xl font-black tracking-tight text-slate-950">Accounting Dashboard</h1>
                <p class="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
                    Monitor guarantee letter cases already routed into accounting and keep the review and approval workload separated without changing the payment stage label.
                </p>
                ...
                <div class="mt-6 flex flex-wrap gap-3">
                    ...
                    @if(! $isOfficer)
                        <a href="{{ route($cashCertificationRoute) }}"
            ...
            <div class="grid gap-3">
                <div class="rounded-3xl border border-slate-200 bg-white/90 px-5 py-5 shadow-sm">
                    ...
                    <p class="mt-2 text-sm text-slate-500">Cases currently inside the accounting stage.</p>
                </div>
                <div class="rounded-3xl border border-sky-200 bg-sky-50/90 px-5 py-5 shadow-sm">
                    ...
                    <p class="mt-2 text-sm text-sky-800">Combined amount currently under accounting handling.</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <article class="rounded-3xl border border-blue-200 bg-blue-50 p-5 shadow-sm">
            <span class="inline-flex rounded-full border border-blue-200 bg-white px-3 py-1 text-[11px] font-black uppercase tracking-[0.16em] text-blue-700">{{ $isOfficer ? 'Review Batches' : 'Approval Batches' }}</span>
            <p class="mt-3 text-3xl font-black text-blue-950">{{ $stats['total'] }}</p>
            <p class="mt-2 text-sm text-blue-800">Active cases in this accounting workspace.</p>
        </article>
        <article class="rounded-3xl border border-sky-200 bg-sky-50 p-5 shadow-sm">
            <span class="inline-flex rounded-full border border-sky-200 bg-white px-3 py-1 text-[11px] font-black uppercase tracking-[0.16em] text-sky-700">
                {{ $isOfficer ? 'Processor Remarks' : 'Accounting Remarks' }}
            </span>
            <p class="mt-3 text-3xl font-black text-sky-950">{{ $isOfficer ? $stats['with_processor_remarks'] : $stats['with_accounting_remarks'] }}</p>
            <p class="mt-2 text-sm text-sky-800">Cases that already include notes for this stage.</p>
        </article>
        ...
        <article class="rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
            ...
            <p class="mt-2 text-sm text-slate-700">All records remain within `For Processing (Accounting)`.</p>
        </article>
        @if(! $isOfficer)
            <article class="rounded-3xl border border-blue-200 bg-blue-50 p-5 shadow-sm">
                <span class="inline-flex rounded-full border border-blue-200 bg-white px-3 py-1 text-[11px] font-black uppercase tracking-[0.16em] text-blue-700">From Cash</span>
                <p class="mt-3 text-3xl font-black text-blue-950">{{ $stats['cash_certifications_total'] }}</p>
                <p class="mt-2 text-sm text-blue-800">Cash-approved cases now waiting for accounting certification.</p>
            </article>
        @endif
    </section>
    ...
    <section class="grid gap-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(320px,.85fr)]">
        <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div class="flex items-center justify-between gap-3">
                <div>
                    ...
                    <h2 class="mt-2 text-2xl font-black text-slate-950">Latest Accounting Cases</h2>
                </div>
                <a href="{{ route($listRoute) }}"
            ...
            <div class="mt-6 space-y-4">
                ...
                @empty
                    <div class="rounded-3xl border border-dashed border-slate-300 bg-slate-50 px-6 py-10 text-center text-sm text-slate-500">
                        No accounting cases are waiting right now.
                    </div>
                @endforelse
            </div>
        </section>
        ...
        @if(! $isOfficer)
            <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex items-center justify-between gap-3">
                    <div>
                        <p class="text-xs font-black uppercase tracking-[0.18em] text-slate-500">From Cash</p>
                        <h2 class="mt-2 text-2xl font-black text-slate-950">Latest Cash Certifications</h2>
                    </div>
                    <a href="{{ route($cashCertificationRoute) }}"
                       class="inline-flex items-center rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
                    Open Batches
                ...
                <div class="mt-5 space-y-3">
                    @forelse($cashCertificationCases as $application)
                        <article class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <p class="mt-3 text-sm font-bold text-slate-900">{{ trim(($application->client?->first_name ?? '').' '.($application->client?->last_name ?? '')) ?: '-' }}</p>
                                    <p class="mt-1 text-sm text-slate-500">{{ $application->serviceProvider?->name ?? 'No provider assigned' }}</p>
                                </div>
                                <a href="{{ route($cashCertificationRoute.'.show', $application->id) }}"
                                   class="inline-flex items-center rounded-xl bg-[#234E70] px-4 py-2 text-sm font-semibold text-white hover:bg-[#18384f]">
                                    Certify
                                </a>
                            </div>
                    ...
                    @empty
                        <div class="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-5 py-6 text-sm text-slate-500">
                            No cash certifications are waiting right now.
                        </div>
                    @endforelse
                </div>
            </section>
