...
@php
    ...
    $paymentStatusLabel = $isCashCertifier ? 'For Processing (Accounting Certification)' : 'For Processing (Accounting)';
                ...
                <form method="POST" action="{{ route($updateRoute, $application->id) }}" class="mt-5 space-y-4">
                    ...
                    @if($isOfficer)
                        <div>
                            ...
                            <textarea name="gl_accounting_remarks" class="input h-32" placeholder="Optional accounting remarks before endorsing to the accounting approver.">{{ old('gl_accounting_remarks', $application->gl_accounting_remarks) }}</textarea>
                        </div>
                        <button type="submit" class="btn-primary">Submit to Accounting Approval</button>
                    @else
                        <div>
