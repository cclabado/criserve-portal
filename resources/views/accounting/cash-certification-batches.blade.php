...
<main class="p-8 max-w-7xl mx-auto space-y-6">
    <section class="rounded-[28px] bg-[radial-gradient(circle_at_top_left,_rgba(255,255,255,0.16),_transparent_35%),linear-gradient(135deg,_#234E70_0%,_#18384f_46%,_#27597c_100%)] px-8 py-9 text-white shadow-[0_24px_60px_rgba(24,56,79,0.18)]">
        <div class="grid gap-6 lg:grid-cols-[1.4fr_.9fr] lg:items-end">
            <div>
                ...
                <h1 class="mt-5 text-3xl font-bold leading-tight sm:text-4xl">Accounting Certification Batches</h1>
                <p class="mt-3 max-w-2xl text-sm leading-6 text-white/80 sm:text-base">
                    Approve and certify grouped GL finance batches after the cash approver endorsement.
                </p>
            </div>
    ...
    <section class="rounded-3xl border border-slate-200 bg-white p-3 shadow-sm">
        <div class="flex flex-wrap gap-3">
            <a href="{{ route('accounting-approver.cash-certifications', ['scope' => 'active']) }}"
            ...
            <a href="{{ route('accounting-approver.cash-certifications', ['scope' => 'finished']) }}"
    ...
    <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <form method="GET" action="{{ route('accounting-approver.cash-certifications') }}" class="grid gap-4 md:grid-cols-4">
            ...
            <div class="flex items-end gap-3">
                ...
                <a href="{{ route('accounting-approver.cash-certifications') }}" class="btn-secondary">Reset</a>
            </div>
        </form>
    </section>
            ...
            <table class="min-w-full text-left">
                ...
                <tbody class="divide-y divide-slate-200">
                    @forelse($batches as $batch)
                        @php
                            $isHistoricalRow = (int) ($batch->accounting_certified_by ?? 0) === (int) auth()->id()
                                && ! in_array($batch->accounting_certification_status, ['pending_approval', null], true);
                            $rowStatusLabel = match ($batch->status) {
                                'for_compliance_cash_officer' => 'For Compliance (Cash Officer)',
                                'for_processing_finance_director' => 'For Processing (Finance Director)',
                                'disapproved' => 'Disapproved',
                                default => 'For Processing (Accounting Certification)',
                            };
                            $rowStatusClass = match ($rowStatusLabel) {
                                'For Compliance (Cash Officer)' => 'border-rose-200 bg-rose-50 text-rose-700',
                                'For Processing (Finance Director)' => 'border-amber-200 bg-amber-50 text-amber-700',
                        ...
                        <tr class="hover:bg-slate-50/70">
                            ...
                            <td class="px-5 py-4 text-right">
                                <a href="{{ route('accounting-approver.cash-certifications.show', $batch->id) }}"
                                   class="inline-flex items-center rounded-xl bg-[#234E70] px-4 py-2 text-sm font-semibold text-white hover:bg-[#18384f]">
                                    {{ $isFinishedScope || $isHistoricalRow ? 'View Batch' : 'Certify Batch' }}
                                </a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="8" class="px-5 py-12 text-center text-sm text-slate-500">
                                No accounting certification batches are waiting right now.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
