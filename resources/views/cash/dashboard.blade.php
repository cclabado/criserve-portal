...
@php
    ...
    $listRoute = $isOfficer ? 'cash-officer.gl-payment-reviews' : 'cash-approver.gl-payment-approvals';
    $listLabel = $isOfficer ? 'Cash Review Batches' : 'Cash Approval Batches';
@endphp
...
<main class="space-y-6">
    <section class="overflow-hidden rounded-[32px] border border-slate-200 bg-[radial-gradient(circle_at_top_left,_rgba(191,219,254,.55),_transparent_28%),linear-gradient(135deg,_#ffffff_0%,_#eef6ff_48%,_#f8fafc_100%)] p-6 shadow-sm lg:p-8">
        <div class="grid gap-6 xl:grid-cols-[minmax(0,1.25fr)_340px] xl:items-end">
            <div>
                ...
                <h1 class="mt-3 text-4xl font-black tracking-tight text-slate-950">Cash Dashboard</h1>
                <p class="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
                    Monitor guarantee letter cases already routed into cash processing and keep the review and approval workload separated while the payment status stays within the cash stage.
                </p>
            ...
            <div class="grid gap-3">
                <div class="rounded-3xl border border-slate-200 bg-white/90 px-5 py-5 shadow-sm">
                    ...
                    <p class="mt-2 text-sm text-slate-500">Cases currently inside the cash stage.</p>
                </div>
                <div class="rounded-3xl border border-sky-200 bg-sky-50/90 px-5 py-5 shadow-sm">
                    ...
                    <p class="mt-2 text-sm text-sky-800">Combined amount currently under cash handling.</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <article class="rounded-3xl border border-blue-200 bg-blue-50 p-5 shadow-sm">
            ...
            <p class="mt-2 text-sm text-blue-800">Active cases in this cash workspace.</p>
        </article>
        <article class="rounded-3xl border border-sky-200 bg-sky-50 p-5 shadow-sm">
            <span class="inline-flex rounded-full border border-sky-200 bg-white px-3 py-1 text-[11px] font-black uppercase tracking-[0.16em] text-sky-700">
                {{ $isOfficer ? 'Previous Remarks' : 'Cash Remarks' }}
            </span>
            <p class="mt-3 text-3xl font-black text-sky-950">{{ $stats['with_previous_remarks'] }}</p>
        ...
        <article class="rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
            ...
            <p class="mt-2 text-sm text-slate-700">All records remain within `For Processing (Cash)`.</p>
        </article>
    </section>
        ...
        <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div class="flex items-center justify-between gap-3">
                <div>
                    ...
                    <h2 class="mt-2 text-2xl font-black text-slate-950">Latest Cash Cases</h2>
                </div>
                <a href="{{ route($listRoute) }}"
